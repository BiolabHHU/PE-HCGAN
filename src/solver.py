...
class Solver(object):
    def __init__(self, data, models, optimizers, args):
        ...
        # Checkpoints
        self.continue_from = args.continue_from     # 是否从某个检查点继续训练
        self.eval_every = args.eval_every       # 每隔多少轮次评估一次
        self.cross_valid = args.cross_valid       # 是否进行交叉验证
        self.cross_valid_every = args.cross_valid_every       # 每隔多少轮次进行交叉验证
        self.checkpoint = args.checkpoint       # 检查点目录
        if self.checkpoint:
    ...
    def _copy_models_states(self):  # 复制并返回所有模型的状态字典（state_dict）
        states = {}
        for name, model in self.models.items():
            states[name] = copy_state(model.state_dict())
        return states

    def _load(self, package, load_best=False):  # 从给定的包（package）中加载模型和优化器的状态
        if load_best:
    ...
    def _reset(self):   # 重置Solver类的某些状态，并在需要时从检查点（checkpoint）或继续点（continue point）加载模型和优化器的状态
        """_reset."""
        load_from = None
        load_best = False
        keep_history = True
    ...
    def _run_one_epoch(self, epoch, cross_valid=False):     # 训练验证一个epoch
        total_losses = {}
        total_loss = 0
        ...
        for i, data in enumerate(logprog):
            ...
            if not cross_valid:
                ...
                # 更新学习率调度器
                # if 'scheduler' in self.optimizers and self.optimizers['scheduler'] is not None:
    ...
    # 在测试数据上运行一个“epoch”并收集损失，同时返回增强文件的名称
    def _get_valid_losses_on_test_data(self, epoch, enhance):
        ...
        for i, data in enumerate(logprog):
            ...
            if self.args.experiment.model == 'ssrm':
    ...
    def _get_losses(self, hr, pr):      # 根据模型计算不同损失
        hr_time = hr['time']
        pr_time = pr['time']
    ...
    def _get_melgan_generator_loss(self, discriminator_fake, discriminator_real):
        ...
        if 'only_adversarial_loss' in self.args.experiment and self.args.experiment.only_adversarial_loss:
            return {'adversarial': self.args.experiment.adversarial_loss_lambda * adversarial_loss}
